...
export const InvoiceDetailPage: React.FC = () => {
  ...
  if (!order) {
    return (
      <div className="py-16 text-center space-y-4 font-mono">
        <h2 className="font-display text-2xl font-bold text-[#111111]">Invoice Not Found</h2>
        <p className="text-xs text-[#4A4844]">The tax invoice record does not exist in Deny OS.</p>
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="max-w-4xl mx-auto bg-[#D5D5D8] border border-[rgba(0,0,0,0.18)] p-8 sm:p-12 space-y-8 font-mono shadow-subtle print:border-none print:shadow-none print:p-0">
        {/* Header */}
        <div className="flex flex-col sm:flex-row justify-between items-start border-b border-[#111111] pb-6 gap-6">
          <div>
            <div className="font-display font-black text-3xl sm:text-4xl tracking-tighter text-[#111111]">
              STUDIO DENY
            </div>
            <div className="text-[11px] text-[#4A4844] uppercase tracking-widest mt-1">
          ...
          <div className="text-left sm:text-right space-y-1">
            ...
            <div className="text-xs text-[#4A4844]">
              Order Ref: <span className="font-bold text-[#111111]">{order.orderNumber}</span>
            </div>
            <div className="text-xs text-[#4A4844]">Date: {order.createdAt.substring(0, 10)}</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 text-xs">
          <div className="p-4 bg-[#D5D5D8] border border-[rgba(0,0,0,0.18)] space-y-1">
            <div className="text-[10px] text-[#4A4844] uppercase tracking-wider">BILLED TO (PATRON):</div>
            <div className="font-bold text-sm text-[#111111]">{order.customerName}</div>
            <div className="text-[#4A4844]">{order.customerEmail}</div>
            <div className="text-[#4A4844]">{order.customerPhone}</div>
          </div>

          <div className="p-4 bg-[#D5D5D8] border border-[rgba(0,0,0,0.18)] space-y-1">
            <div className="text-[10px] text-[#4A4844] uppercase tracking-wider">DISPATCH DESTINATION:</div>
            <div className="font-semibold text-[#111111]">{order.shippingAddress.street}</div>
            <div className="text-[#4A4844]">
              {order.shippingAddress.city}, {order.shippingAddress.state} - {order.shippingAddress.pincode}
            </div>
            <div className="text-[#4A4844]">{order.shippingAddress.country}</div>
          </div>
        </div>

        {/* Items Table */}
        <div className="border border-[rgba(0,0,0,0.18)] overflow-hidden">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-[rgba(0,0,0,0.1)]">
              {order.items.map((item, idx) => (
                <tr key={idx}>
                  <td className="py-3 px-4 text-[#4A4844]">61091000</td>
                  ...
                  <td className="py-3 px-4 text-right font-bold text-[#111111]">{formatINR(item.total)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start gap-6 pt-2">
          <div className="text-xs space-y-2 max-w-sm">
            <div className="p-3 bg-[#E2E2E4] border border-[rgba(0,0,0,0.18)] space-y-1">
              <span className="text-[10px] text-[#4A4844] uppercase block">TENDER SETTLEMENT</span>
          ...
          <div className="w-full sm:w-72 space-y-2 text-xs">
            ...
            <div className="flex justify-between items-baseline pt-2 border-t border-[#111111] text-sm">
              <span className="font-bold text-[#111111]">GRAND TOTAL:</span>
              <span className="font-black text-xl text-[#111111]">{formatINR(order.grandTotal)}</span>
            </div>
          </div>
        </div>

        {/* Footer Notes */}
        <div className="border-t border-[rgba(0,0,0,0.1)] pt-4 text-center text-[10px] text-[#4A4844] space-y-1">
          <div>This is a computer generated tax invoice in accordance with CGST/SGST Act 2017.</div>
          <div className="tracking-widest uppercase font-semibold text-[#111111]">
