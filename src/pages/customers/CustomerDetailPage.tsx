  ...
  if (!customer) {
    return (
      <div className="py-16 text-center space-y-4">
        <h2 className="font-display text-2xl font-bold text-[#111111]">Customer Not Found</h2>
        <p className="text-xs font-mono text-[#4A4844]">No customer profile exists matching this ID.</p>
    ...
    <div className="space-y-8 animate-in fade-in duration-200">
      ...
      <div className="bg-[#D5D5D8] border border-[rgba(0,0,0,0.18)] p-6">
        {isEditing ? (
          <form onSubmit={handleSaveCustomer} className="grid grid-cols-1 sm:grid-cols-2 gap-4 font-mono text-xs">
            <div>
              <label className="block text-[10px] uppercase tracking-widest text-[#4A4844] mb-1">Full Name</label>
              <input
                required
                value={editName}
                onChange={(e) => setEditName(e.target.value)}
                className="w-full bg-[#D5D5D8] border border-[rgba(0,0,0,0.18)] p-2 text-xs font-mono focus:outline-none focus:border-[#111111]"
              />
            </div>
            <div>
              <label className="block text-[10px] uppercase tracking-widest text-[#4A4844] mb-1">Phone</label>
              <input
                required
                value={editPhone}
            ...
            <div>
              <label className="block text-[10px] uppercase tracking-widest text-[#4A4844] mb-1">Email</label>
              <input
                type="email"
                value={editEmail}
            ...
            <div>
              <label className="block text-[10px] uppercase tracking-widest text-[#4A4844] mb-1">City</label>
              <input
                value={editCity}
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div className="flex items-start gap-4">
              <div className="w-16 h-16 bg-[#111111] text-[#E2E2E4] flex items-center justify-center font-display font-black text-2xl shrink-0">
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center gap-4 text-xs font-mono text-[#4A4844] mt-2">
                  <span className="flex items-center gap-1.5">
                    <Mail size={13} className="text-[#4A4844]" /> {customer.email}
                  </span>
                  <span className="flex items-center gap-1.5">
                    <Phone size={13} className="text-[#4A4844]" /> {customer.phone}
                  </span>
                  <span className="flex items-center gap-1.5">
            ...
            <div className="border-t lg:border-t-0 lg:border-l border-[rgba(0,0,0,0.18)] pt-4 lg:pt-0 lg:pl-6 space-y-1">
              <span className="text-[10px] font-mono uppercase tracking-widest text-[#4A4844] block">
                PRIMARY SHIPPING COORDINATES
              </span>
              <p className="text-xs font-mono text-[#111111] max-w-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="bg-[#D5D5D8] border border-[rgba(0,0,0,0.18)] overflow-hidden">
            <div className="p-4 border-b border-[rgba(0,0,0,0.18)] flex items-center justify-between bg-[#E2E2E4]">
              <div className="flex items-center gap-2">
                <ShoppingBag size={15} className="text-[#111111]" />
            ...
            {customerOrders.length === 0 ? (
              <div className="py-10 text-center text-xs font-mono text-[#4A4844]">
                ...
                <table className="w-full text-left border-collapse font-mono text-xs">
                  ...
                  <tbody className="divide-y divide-[rgba(0,0,0,0.1)]">
                    {customerOrders.map((ord) => (
                      <tr key={ord.id} className="hover:bg-[#E2E2E4] transition-colors">
                        <td className="py-3 px-4 font-bold text-[#111111]">{ord.orderNumber}</td>
                        <td className="py-3 px-4 text-[#4A4844]">{ord.createdAt.substring(0, 10)}</td>
                        <td className="py-3 px-4 text-[#111111]">
                          {ord.items.map((i) => `${i.name} (${i.size})`).join(', ')}
                        </td>
                        <td className="py-3 px-4 font-bold text-[#111111]">{formatINR(ord.grandTotal)}</td>
          ...
          {customerReturns.length > 0 && (
            <div className="bg-[#D5D5D8] border border-[rgba(0,0,0,0.18)] overflow-hidden">
              <div className="p-4 border-b border-[rgba(0,0,0,0.18)] flex items-center justify-between bg-[#E2E2E4]">
                <div className="flex items-center gap-2">
                  <RotateCcw size={15} className="text-[#111111]" />
                  <h3 className="font-mono text-xs font-bold uppercase tracking-wider text-[#111111]">
                    RETURNS & SIZE EXCHANGES ({customerReturns.length})
                  </h3>
                </div>
              </div>
              <div className="divide-y divide-[rgba(0,0,0,0.1)] font-mono text-xs">
                {customerReturns.map((ret) => (
                  <div key={ret.id} className="p-4 flex items-center justify-between hover:bg-[#E2E2E4]">
                    <div>
                      <div className="font-bold text-[#111111]">{ret.returnNumber}</div>
                      <div className="text-[11px] text-[#4A4844] mt-0.5">
                        {ret.productTitle} — {ret.variantName}
                      </div>
                      <div className="text-[10px] text-[#4A4844] mt-1">Reason: {ret.reason}</div>
                    </div>
        ...
        <div className="lg:col-span-4 space-y-4">
          <div className="bg-[#111111] text-[#E2E2E4] p-5 border border-[#111111] space-y-3">
            ...
            <div className="pt-2 border-t border-neutral-800 space-y-1.5 font-mono text-[11px]">
              <div className="flex justify-between text-neutral-400">
                ...
                <span className="text-[#E2E2E4] font-bold">{topItem || 'N/A'}</span>
              </div>
              <div className="flex justify-between text-neutral-400">
                <span>PREFERRED PAYMENT:</span>
                <span className="text-[#E2E2E4] font-bold">{topPayment || 'N/A'}</span>
              </div>
            </div>
          </div>
        </div>
